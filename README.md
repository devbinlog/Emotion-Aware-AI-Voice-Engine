...
---
  ...
  │           구간            │          상태           │
  ├───────────────────────────┼─────────────────────────┤
  │ 마이크 → WebSocket 전송   │ ✅ 안정적               │
  ├───────────────────────────┼─────────────────────────┤
  │ ffmpeg WebM 디코딩        │ ✅                      │
  ├───────────────────────────┼─────────────────────────┤
  │ STT (faster-whisper base) │ ✅ 한국어/영어 인식     │
  ├───────────────────────────┼─────────────────────────┤
  │ 감정 분석 (룰 기반)       │ ⚠️  동작하나 정확도 낮음 │
  ├───────────────────────────┼─────────────────────────┤
  │ LLM 대화 (qwen2.5:1.5b)   │ ⚠️  느리고 말투 어색     │
  ├───────────────────────────┼─────────────────────────┤
  │ TTS + prosody 후처리      │ ✅                      │
  ├───────────────────────────┼─────────────────────────┤
  │ 날씨 검색 (Open-Meteo)    │ ✅                      │
  ...
  1. 응답 속도 — LLM이 CPU에서 7~12초. 실사용 불가 수준
  2. 대화 말투 — 1.5B 소형 모델 한계로 어색한 표현 잔존
  3. 감정 인식 정확도 — 룰 기반이라 실제 감정과 불일치 많음
  4. 마이크 의존성 — 일반 내장 마이크에서 RMS가 낮아 감정 감지 불안정
  ...
  - LLM 모델 교체 — ollama pull qwen2.5:7b 또는 llama3.1:8b. 약 4~5GB지만 대화 품질이 훨씬 자연스러움
  - GPU 가속 — Metal(macOS) 또는 CUDA 지원 시 LLM 응답 1~2초대로 단축 가능
  - 마이크 환경 개선 — 외장 마이크 사용 시 감정 분석 입력 품질 향상

  중기 (구현 필요)

  - 감정 분석 ML 전환 — 룰 기반 → speechbrain ECAPA-TDNN 또는 emotion2vec 같은 학습된 SER 모델. 정확도 대폭 향상
  - 텍스트 감정 분석 강화 — 키워드 렉시콘 → klue/roberta 또는 KoELECTRA fine-tuned 감정 분류 모델
  - 스트리밍 STT — 녹음 중 실시간 부분 트랜스크립트 표시 (현재는 녹음 후 일괄 처리)

  장기 (큰 개선)

  - 한국어 전용 TTS — macOS say 대신 XTTS v2 또는 한국어 VITS. 훨씬 자연스러운 음성
  - 감정 반영 LLM 프롬프트 강화 — 현재 감정 정보를 컨텍스트로만 넘기는데, 감정별로 응답 전략 다르게 구성
  - 실시간 파이프라인 — 발화 중 STT → LLM 스트리밍 → TTS 동시 처리로 체감 지연 대폭 감소
