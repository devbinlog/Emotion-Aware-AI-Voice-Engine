...
> **포트폴리오 프로젝트** — AI Sound & Voice Engineer
...
사람마다 목소리에 감정이 담겨 있다. 이 프로젝트는 사용자의 음성에서 **감정 상태를 실시간으로 감지**하고, 감지된 감정에 맞는 **운율(prosody)로 응답 음성을 합성**하는 엔드-투-엔드 파이프라인을 구현한다.
...
**정의**: 오디오 신호의 평균 진폭 에너지. 쉽게 말해 **목소리의 크기(음량)**.
...
- 텍스트는 **명시적 감정 표현**만 잡아내므로 보조 역할(0.4)
- 텍스트가 없으면 오디오 결과만 사용
...
| **STT 품질** | whisper `small` 또는 `medium` 전환 |
| **한국어 TTS** | XTTS v2 (GPU) 또는 한국어 VITS fine-tune |
| **스트리밍 STT** | faster-whisper streaming → 실시간 부분 텍스트 |
| **화자 분리** | pyannote-audio 스피커 트래킹 |
